...
-) 选定一对登录帐号，一个用于工具箱（TOOLBOX_ACCOUNT），一个用于APP（ISTUDIO_ACCOUNT），
   这个要修改 MyApplication.java 里面的 ISTUDIO_ACCOUNT TOOLBOX_ACCOUNT 变量
   如果要用聊天，演示提供私聊，i052Chatactivity.java 里面的对方（工具箱登录的PID）要改
   
-）新的界面Activity 和相关类必须用 ixxx 开头，i小写，xxx 是界面编号

-）这个命名规则包括界面用到的资源，控件ID, layout xml文件名字等

-）拿到工程后，首先确保 MainActivity 里面的改到项目的服务器地址
 
	APIConnection.wsURL = "ws://112.124.70.60:51727/demo";

-）修改 package 名字和 build.gradle 里面的AppID 
